...
type TaskType = 'odd-even' | 'high-low';

interface TaskSwitchingSessionReportProps {
  readonly report?: SessionEndReportModel | null;
  readonly trials?: readonly TaskSwitchingTrialView[];
...
interface TaskSwitchingTrialView {
  readonly index: number;
  readonly taskType: TaskType;
  readonly number: number;
  readonly isSwitch: boolean;
  readonly response: 'left' | 'right' | null;
  readonly correctResponse: 'left' | 'right';
...
function isTaskType(value: unknown): value is TaskType {
  return value === 'odd-even' || value === 'high-low';
}

export function TaskSwitchingSessionReport({
  report,
  trials,
  totalTrials,
  accuracy,
  ...
  onPlayAgain,
  onBackToHome,
}: TaskSwitchingSessionReportProps): ReactNode {
  const { t } = useTranslation();

  const resolvedTrials = useMemo<TaskSwitchingTrialView[]>(() => {
    ...
    return reportTurns
      ...
      .map((turn) => {
        ...
        const taskType = isTaskType(trialData['task']) ? trialData['task'] : 'odd-even';
        const digit = typeof trialData['digit'] === 'number' ? trialData['digit'] : 0;
        const isSwitch = trialData['isSwitch'] === true;
        const response =
          trialData['response'] === 'left' || trialData['response'] === 'right'
            ? (trialData['response'] as 'left' | 'right')
            : null;
        const timedOut = trialData['timedOut'] === true;

        // Infer correct response from digit and task
        let correctResponse: 'left' | 'right';
        if (taskType === 'odd-even') {
          correctResponse = digit % 2 === 0 ? 'left' : 'right';
        } else {
          correctResponse = digit > 5 ? 'right' : 'left';
        }
  ...
  const resolvedSwitchCost =
    switchCostMs ??
    (typeof report?.taskMetrics?.['switchCostMs'] === 'number'
      ? Math.round(report.taskMetrics['switchCostMs'])
      : (() => {
          const switchTrials = resolvedTrials.filter(
            (turn) => turn.isSwitch && turn.correct && !turn.timedOut,
          );
          const repeatTrials = resolvedTrials.filter(
            (turn) => !turn.isSwitch && turn.correct && !turn.timedOut,
          );
          const meanSwitch =
            switchTrials.length > 0
              ? switchTrials.reduce((sum, turn) => sum + turn.responseTimeMs, 0) /
                switchTrials.length
              : 0;
          const meanRepeat =
            repeatTrials.length > 0
              ? repeatTrials.reduce((sum, turn) => sum + turn.responseTimeMs, 0) /
                repeatTrials.length
              : 0;
          return Math.round(meanSwitch - meanRepeat);
        })());

  const conditionStats = useMemo(() => {
    return [false, true].map((isSwitch) => {
      const matchingTurns = resolvedTrials.filter((turn) => turn.isSwitch === isSwitch);
      ...
      return {
        id: isSwitch ? 'switch' : 'repeat',
        label: isSwitch
          ? t('report.taskSwitching.switch', 'Switch')
          : t('report.taskSwitching.repeat', 'Repeat'),
  ...
  return (
    <div className="w-full md:max-w-md lg:max-w-lg md:mx-auto">
      <Hatching id="task-switching-report-top" className="text-foreground/70" />
      <div className="flex items-stretch gap-x-2">
        <Hatching
          id="task-switching-report-left"
        ...
        <div className="min-w-0 flex-1">
          <div className="px-2 pt-4 pb-0 text-center">
            <h2 className="text-2xl sm:text-3xl font-black tracking-tight leading-tight text-primary">
              {t('report.taskSwitching.title', 'Task Switching')}
            </h2>
            <p className="mt-1 text-xs uppercase tracking-[0.25em] text-woven-text-muted">
              {t('report.taskSwitching.subtitle', 'Cognitive flexibility')}
            </p>
            <Hatching id="task-switching-report-hero" className="mt-2 text-foreground/70" />
          </div>
          ...
          <Hatching id="task-switching-report-score-hatch" className="mt-3 text-foreground/70" />
          ...
          <div className="px-2 mt-4 grid grid-cols-4 gap-2 text-center">
            ...
            <div className="rounded-xl border border-woven-border/60 bg-woven-surface/80 backdrop-blur-lg shadow-sm p-2.5">
              <p className="text-[10px] uppercase tracking-wide text-woven-text-muted">
                {t('report.taskSwitching.switchCostShort', 'Switch Cost')}
              </p>
              <p className="text-lg font-bold tabular-nums text-primary">{resolvedSwitchCost}ms</p>
            </div>
          </div>
          <Hatching id="task-switching-report-kpi-hatch" className="mt-4 text-foreground/70" />

          <div className="px-2 py-4 space-y-2">
            <Disclosure
              title={t('report.taskSwitching.performanceByCondition', 'Performance by condition')}
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <span
                          className={cn(
                            ...
                            condition.id === 'repeat' ? 'bg-woven-correct' : 'bg-primary',
                          )}
                        />
                      ...
                      <div className="mt-3 space-y-1.5 text-xs text-woven-text-muted">
                        ...
                        <div className="flex items-center justify-between border-t border-woven-border/60 pt-1.5">
                          <span>{t('report.taskSwitching.precision', 'Precision')}</span>
            ...
            <Disclosure
              title={t('report.taskSwitching.timeline', 'Timeline')}
                ...
                <div className="relative pl-5">
                  ...
                  <div className="space-y-3">
                    {resolvedTrials.map((turn) => {
                      const taskLabel =
                        turn.taskType === 'odd-even'
                          ? t('report.taskSwitching.oddEven', 'Odd/Even')
                          : t('report.taskSwitching.highLow', 'High/Low');
                        ...
                        <div key={turn.index} className="relative">
                          ...
                          <div className="rounded-xl border border-woven-border/60 bg-woven-surface/80 backdrop-blur-lg shadow-sm px-3 py-3">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-lg font-black tabular-nums text-woven-text">
                                {turn.number}
                              </span>
                              <span
                                className={cn(
                            ...
                            <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] text-woven-text-muted">
                              {turn.response ? (
                                <span className="rounded-full bg-woven-bg px-2 py-1">
                                  {t('report.taskSwitching.response', 'Response')}:{' '}
                              ...
                              >
                                {turn.timedOut
                                  ? t('report.taskSwitching.timeout', 'Timeout')
                                  : turn.correct
                                    ? t('report.taskSwitching.correct', 'Correct')
                                    : t('report.taskSwitching.error', 'Error')}
                              </span>
                            </div>
                          </div>
                        </div>
          ...
          <Hatching id="task-switching-report-actions-hatch" className="text-foreground/70" />
        ...
        <Hatching
          id="task-switching-report-right"
          orientation="vertical"
          className="shrink-0 text-foreground/70"
        />
      </div>
      <Hatching id="task-switching-report-bottom" className="text-foreground/70" />
    </div>
  );
}
