/**
 * PlaceSettingsOverlay.tsx - Mini menu de paramètres en overlay
 *
...
interface PlaceSettingsOverlayProps {
  isOpen: boolean;
  onClose: () => void;
...
export function PlaceSettingsOverlay({
  isOpen,
  onClose,
  onHaptic,
  showModalityLabels,
      ...
      <div className="relative bg-woven-surface/95 dark:bg-woven-surface backdrop-blur-md dark:backdrop-blur-none border border-woven-border rounded-2xl shadow-xl p-4 min-w-[280px] max-w-[90vw] animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="space-y-1">
          <ScaleControl
            ...
            onHaptic={onHaptic}
            label={t('flow.settings.gridScale', 'Grid size')}
          />
          <ToggleSwitch
            checked={countdownMode}
            onChange={onCountdownModeChange}
            onHaptic={onHaptic}
            label={t('flow.settings.countdownMode', 'Countdown (instead of counter)')}
          />
          <ToggleSwitch
            checked={showNLevel}
          ...
          <ToggleSwitch
            ...
            onHaptic={onHaptic}
            label={t('flow.settings.showAdaptiveZone', 'Show adaptive zone')}
          />
          ...
          <ToggleSwitch
            ...
            onHaptic={onHaptic}
            label={t('flow.settings.modalityLabels', 'Position / Audio labels')}
          />
          <ToggleSwitch
            checked={showTimeLabels}
            onChange={onShowTimeLabelsChange}
            onHaptic={onHaptic}
            label={t('flow.settings.timeLabels', 'Present / Past labels')}
          />
          <ToggleSwitch
            checked={showRecenterButton}
            onChange={onShowRecenterButtonChange}
            onHaptic={onHaptic}
            label={t('flow.settings.recenterButton', 'Recenter button')}
          />
