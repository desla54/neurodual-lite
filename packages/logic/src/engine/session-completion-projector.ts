...
import type { GameEvent, MotEvent, SessionSummary } from './events';
...
import { projectTrackTurns, projectCognitiveTaskTurns, projectCorsiTurns } from './turn-projectors';
...
export class SessionCompletionProjector {
  ...
  static project(input: SessionCompletionInput): SessionCompletionResult | null {
    switch (input.mode) {
      ...
      case 'time':
        return SessionCompletionProjector.projectTime(input);
      case 'track':
        return SessionCompletionProjector.projectTrack(input);
      case 'corsi':
        return SessionCompletionProjector.projectCorsi(input);
      case 'ospan':
        return SessionCompletionProjector.projectOspan(input);
      case 'running-span':
        return SessionCompletionProjector.projectRunningSpan(input);
      case 'pasat':
        return SessionCompletionProjector.projectPasat(input);
      case 'swm':
        return SessionCompletionProjector.projectSwm(input);
      case 'cognitive-task':
  ...
  private static projectTime(input: TimeCompletionInput): SessionCompletionResult | null {
    const accuracy = input.accuracy / 100; // normalize 0-1
    ...
    const passed = completed && accuracy >= AllSpecs['dual-time'].scoring.passThreshold;
    const nextLevel = 1; // Time mode doesn't use N-levels
    ...
    const report: SessionEndReportModel = {
      ...
      passed,
      totals: {
        hits: input.successfulTrials,
        misses: input.failedTrials,
        falseAlarms: null,
      ...
      turns: projectCorsiTurns(input.events),
    };

    const reportWithPlayContext = enrichReportWithEventContext(report, input.events);

    // Build a TraceSessionSummary-compatible shape for XP calculation
    const summary: TraceSessionSummary = {
      ...
      responses: [],
      finalStats: {
        trialsCompleted: input.trialsCompleted,
        warmupTrials: 0,
        correctResponses: input.successfulTrials,
        incorrectResponses: input.failedTrials,
        timeouts: 0,
        accuracy,
      },
    };
  ...
  private static projectTrack(input: TrackCompletionInput): SessionCompletionResult | null {
    const projection = projectTrackSessionFromEvents(input.events);
    if (!projection) return null;
    const trackEvents = input.events.filter((event): event is MotEvent => {
      return (
    ...
    const report: SessionEndReportModel = {
      ...
      errorProfile: {
        errorRate:
          projection.totalTargetsPresented + projection.totalFalseAlarms > 0
    ...
    const summary: TraceSessionSummary = {
      ...
      score: Math.round(projection.accuracyPercent),
      responses: [],
      finalStats: {
        trialsCompleted: projection.completedTrials,
        warmupTrials: 0,
        correctResponses: projection.totalHits,
        incorrectResponses: projection.totalMisses + projection.totalFalseAlarms,
        timeouts: 0,
    ...
    return {
      ...
      ups: projection.ups,
      passed: projection.passed,
      nextLevel: projection.targetCount,
      summary,
      report: reportWithPlayContext,
      journeyContext: input.journeyContext,
      activeModalities: ['position'],
  ...
  private static projectCorsi(input: CorsiCompletionInput): SessionCompletionResult | null {
    const accuracy = input.accuracy / 100;
    const completed = input.reason === 'completed';
    const ups = UnifiedScoreCalculator.calculate(
      input.accuracy,
      input.maxSpan > 0 ? (input.maxSpan / 9) * 100 : null,
      false,
      'corsi-block',
    );
    const passed = completed && accuracy >= AllSpecs['corsi-block'].scoring.passThreshold;
    ...
    const report: SessionEndReportModel = {
      ...
      passed,
      totals: {
        hits: input.correctTrials,
        misses: Math.max(0, input.totalTrials - input.correctTrials),
        falseAlarms: null,
        correctRejections: null,
      },
      byModality: {},
  ...
  private static projectRunningSpan(
    input: RunningSpanCompletionInput,
    ...
    const passed = completed && accuracy >= AllSpecs['running-span'].scoring.passThreshold;
    const nextLevel = input.maxSpan;
    const createdAt = new Date(input.events[0]?.timestamp ?? Date.now()).toISOString();
  ...
  private static projectPasat(input: PasatCompletionInput): SessionCompletionResult | null {
    const accuracy = input.accuracy / 100;
    const projection = projectPasatSessionFromEvents(input.events);
    if (!projection) return null;

    const ups = projection.ups;
    const passed = projection.passed;
    const nextLevel = 1; // PASAT doesn't use N-levels
    const createdAt = new Date(input.events[0]?.timestamp ?? Date.now()).toISOString();
    ...
    const report: SessionEndReportModel = {
      ...
      speedStats: {
        labelKey: 'report.speed.reactionTime',
        valueMs: input.avgResponseTimeMs,
      },
    };
  ...
  private static projectSwm(input: SwmCompletionInput): SessionCompletionResult | null {
    const accuracy = input.accuracy / 100;
    const projection = projectSwmSessionFromEvents(input.events);
    if (!projection) return null;

    const ups = projection.ups;
    const passed = projection.passed;
    const nextLevel = projection.maxSpanReached;
    const createdAt = new Date(input.events[0]?.timestamp ?? Date.now()).toISOString();
    ...
    const report: SessionEndReportModel = {
      ...
      passed,
      totals: {
        hits: input.correctRounds,
        misses: Math.max(0, input.totalRounds - input.correctRounds),
        falseAlarms: null,
        correctRejections: null,
      },
      byModality: {},
    ...
    const summary: TraceSessionSummary = {
      ...
      responses: [],
      finalStats: {
        trialsCompleted: input.totalRounds,
        warmupTrials: 0,
        correctResponses: input.correctRounds,
        incorrectResponses: Math.max(0, input.totalRounds - input.correctRounds),
        timeouts: 0,
        accuracy,
      },
    };
